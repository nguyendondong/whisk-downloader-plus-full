...
async function waitForStableImages(timeout = 45000, targetCount = 2) {
  ...
  while (Date.now() - startTime < timeout) {
    ...
    try {
      const currentUrls = await waitForImagesInGrid(3000, targetCount);
      ...
      (async () => {
        try {
          ...
          if (msg.promptIndex === 1) {
            console.log(`[SUBMIT] First prompt, clearing downloaded URLs list`);
            await clearDownloadedUrls();
          }
          
          console.log(`[SUBMIT] Starting prompt ${msg.promptIndex}: ${msg.prompt}`);
          
          // Step 1: Submit the prompt
          const submitResult = await submitPromptToUI(msg.prompt);
          if (!submitResult.success) {
            throw new Error(`Failed to submit prompt: ${submitResult.error}`);
          }
          
          // Step 2: Wait for 2 images to be fully rendered and stable
          console.log(`[SUBMIT] Prompt submitted, waiting for 2 images to render...`);
          await waitForStableImages(45000, 2);
          
          console.log(`[SUBMIT] Images rendered and stable for prompt ${msg.promptIndex}`);
          sendResponse({ status: "rendered" });
          
        } catch (error) {
          console.error(`[SUBMIT] Error processing prompt ${msg.promptIndex}:`, error);
...
// Handle download first image messages
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === "downloadFirstImage") {
    (async () => {
      try {
        console.log(`[DOWNLOAD_HANDLER] Starting download for prompt ${msg.promptIndex}`);
        const result = await downloadSingleImage(msg.promptIndex);
        sendResponse(result);
      } catch (error) {
        console.error(`[DOWNLOAD_HANDLER] Error:`, error);
        sendResponse({ status: "error", message: error.message });
      }
    })();
    return true;
